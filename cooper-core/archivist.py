...
def index_brain(conn: sqlite3.Connection, brain_dir: Optional[Path] = None, force: bool = False) -> None:
    ...
    if not directory.exists():
        ...
        return
    # Recursive since 2026-09-07: the brain gained subdirectories (codemap/ —
    # one note per runtime module, generated by cooper-core/codemap.py), and a
    # top-level glob would have silently left them unrecallable — the note
    # exists, the graph shows it, recall() never sees it: the silent-empty
    # class again. file_name keys store the path RELATIVE to the brain root
    # ("codemap/jobs.md"), so a subdirectory note can never collide with a
    # top-level one.
    ...
    for path in md_files:
        ...
        try:
            ...
            with _DB_LOCK:
                # legacy rows were keyed by bare basename; clear both keys so a
                # re-index never leaves a stale duplicate under the old name
                conn.execute("DELETE FROM brain_fts WHERE file_name = ?", (rel_name,))
                ...
                for heading, body in _chunk_by_heading(text):
                    conn.execute(
                        ...
                        (rel_name, heading, body),
                    )
                conn.commit()
        except Exception as exc:
            conn.rollback()
            print(f"  [!!] archivist.index_brain: skipping {rel_name} ({exc})")
