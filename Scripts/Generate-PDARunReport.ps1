...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$OutputRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) "PDA-Backups\build-runner\reports"),
...
if (-not $State) {
    throw "Task state not found for run report."
}
...
Write-Host "[OK] PDA build runner report generated."
