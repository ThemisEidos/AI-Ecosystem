...
$GoalPlan = [pscustomobject]@{
    ...
    input_text = $Text
    goal = if ($Text -match '(?i)classic literature') { "Create a classic literature reading guide PDF" } elseif ($Text -match '(?i)roadmap') { "Build a roadmap" } elseif ($Text -match '(?i)study plan') { "Create a study plan" } elseif ($Text -match '(?i)\b(filesystem|file system|repository|repositories|docker|service|tool inventory|organize folders|file structure|environment)\b') { "Analyze the local environment and recommend a file structure" } else { $Text.Trim() }
    goal_type = $GoalType
    category = $Category
