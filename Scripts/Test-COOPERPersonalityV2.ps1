...
try {
    ...
    if ([string]$Initial.prompt -notmatch 'Canonical voice: one stable voice across all slider presets' -or [string]$Initial.prompt -notmatch 'Profile: slider preset only; it does not change the canonical voice' -or [string]$Initial.prompt -notmatch 'Greeting style: terse' -or [string]$Initial.prompt -notmatch 'generic assistant greetings' -or [string]$Initial.prompt -notmatch 'assessment, opinion, judgment' -or [string]$Initial.prompt -notmatch 'Keep conversational answers short' -or [string]$Initial.prompt -notmatch 'Style: lead with the answer\. Surface implications early\. Avoid automatic next steps' -or [string]$Initial.prompt -notmatch 'When asked for an assessment, opinion, judgment, or risk review, answer in 1 to 4 sentences with short paragraphs, observations, tradeoffs, and concerns' -or [string]$Initial.prompt -notmatch 'Do not use section headers like Recommendation, Assessment, Risks, Confidence, Benefits, Costs, or Alternative unless the user explicitly asks for a report, analysis, plan, assessment matrix, risk register, dashboard, or status output' -or [string]$Initial.prompt -notmatch 'Do not automatically provide recommendations, solutions, or next steps' -or [string]$Initial.prompt -notmatch 'Avoid neutral filler such as it depends' -or [string]$Initial.prompt -notmatch 'Do not pretend to execute physical actions' -or [string]$Initial.prompt -notmatch 'unsafe or impossible physical action') {
    ...
    if ([string]$PromptAfterProfile.prompt -notmatch 'Canonical voice: one stable voice across all slider presets' -or [string]$PromptAfterProfile.prompt -notmatch 'Profile: slider preset only; it does not change the canonical voice' -or [string]$PromptAfterProfile.prompt -notmatch 'assessment, opinion, judgment' -or [string]$PromptAfterProfile.prompt -notmatch 'Keep conversational answers short' -or [string]$PromptAfterProfile.prompt -notmatch 'Style: lead with the answer\. Surface implications early\. Avoid automatic next steps' -or [string]$PromptAfterProfile.prompt -notmatch 'Do not automatically provide recommendations, solutions, or next steps' -or [string]$PromptAfterProfile.prompt -notmatch 'Do not pretend to execute physical actions') {
