...
$CommandText = if ($Task.command) { [string]$Task.command } else { "/draft" }
...
$MarkdownOutputFolder = Join-Path $Root "Obsidian Vault\02_Projects\AI Tool Ecosystem\Agent Findings\Drafts"
$ResultPath = Join-Path $Root "PDA-Tasks\results\$TaskId-result.json"
$DefaultPattern = Get-PDAWorkerDefaultPattern -WorkerName "draft-worker"

function New-PDADraftFailure {
    param(
        [string]$Reason,
    ...
    try {
        ...
        Register-PDAWorkerArtifact -ArtifactPath $ResultPath -TaskId $TaskId -WorkerName "draft-worker" -Command $CommandText -Category $CategoryText -ArtifactType "worker_result_json" -Summary "Draft worker canonical result contract"
    }
    catch {
        Write-Warning "Failed to write draft worker failure result artifact: $($_.Exception.Message)"
    }
...
if ([string]::IsNullOrWhiteSpace($SourcePath) -or -not (Test-Path -Path $SourcePath -PathType Leaf)) {
    New-PDADraftFailure -Reason "No valid source_path provided." -OutputType "error"
    return
}

$FindingsContent = Get-Content $SourcePath -Raw
...
$FabricRender = Invoke-PDAFabricPromptRender -PatternName $DefaultPattern -WorkerName "draft-worker" -TaskType "draft" -Sensitivity "standard" -PromptText $Prompt -Category $CategoryText -SourcePath $SourcePath
$PromptSource = "legacy"
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($Content)) {
        throw "Draft model returned empty content."
    }

    New-Item -ItemType Directory -Force -Path $MarkdownOutputFolder | Out-Null
    $Timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $MarkdownPath = Join-Path $MarkdownOutputFolder "draft-output-$Timestamp.md"
    ...
    Register-PDAWorkerArtifact -ArtifactPath $MarkdownPath -TaskId $TaskId -WorkerName "draft-worker" -Command $TaskCommand -Category $CategoryText -ArtifactType "draft_markdown" -Summary "Draft worker markdown output"
    Register-PDAWorkerArtifact -ArtifactPath $ResultPath -TaskId $TaskId -WorkerName "draft-worker" -Command $TaskCommand -Category $CategoryText -ArtifactType "worker_result_json" -Summary "Draft worker canonical result contract"

    $ResultObject | ConvertTo-Json -Depth 20
}
catch {
    New-PDADraftFailure -Reason $_.Exception.Message -OutputType "error"
}
